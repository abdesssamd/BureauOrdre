...
// Restriction R̫le : Seul Admin, Secr̩taire ou Secretaria peut scanner
...
// Mod̬les d'objets pr̩d̩finis
$obj_templates = [];
try {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['ajax_add_contact'])) {
    $allow_without_doc = isset($_POST['without_document']) && $_POST['without_document'] === '1';
    $has_doc = isset($_FILES['document']) && ($_FILES['document']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE;
    $file_id = null;

    if (!$allow_without_doc && !$has_doc) {
        $message = "<div class='alert error'>Veuillez joindre un document ou cocher \"Ordre sans document\".</div>";
    } elseif ($has_doc && ($_FILES["document"]["error"] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        ...
        try {
            ...
            if ($has_doc) {
                ...
                if (!move_uploaded_file($file['tmp_name'], $target_dir . $unique_name)) {
                    throw new Exception("Impossible de d̩placer le fichier.");
                }

                $full_path = $target_dir . $unique_name;
                $ext = strtolower(pathinfo($unique_name, PATHINFO_EXTENSION));
                $extracted_text = null;
                $ocr_status = 'pending';
            ...
            $message .= "<script>setTimeout(function(){ window.location.href = 'registre.php'; }, 2000);</script>";
        } catch (Exception $e) {
...
<script>
    function isWithoutDocument() {
        const cb = document.getElementById('withoutDocument');
        return cb && cb.checked;
    }

    function toggleDocumentRequirement() {
        const disabled = isWithoutDocument();
        ...
        if (dropZone) {
            dropZone.style.opacity = disabled ? '0.55' : '1';
        ...
        if (disabled) {
            ...
            if (fileName) {
                fileName.innerText = 'Aucun document (ordre sans document)';
                fileName.style.color = '#64748b';
            }
            if (scanPreview) scanPreview.innerHTML = '';
        } else if (fileName && fileName.innerText.trim() === 'Aucun document (ordre sans document)') {
            fileName.innerText = 'Cliquez pour choisir un fichier';
            fileName.style.color = '';
        }
    }

    function scanToJpg() {
        if (isWithoutDocument()) {
            alert("Le mode 'Ordre sans document' est actif.");
            return;
        }
        ...
        // J'active la fen̻tre (dialog) ici aussi pour que vous puissiez choisir
    ...
    function scanToPdf() {
        ...
        // ICI C'EST IMPORTANT : use_asprise_dialog = TRUE
        // Cela ouvre la fen̻tre du scanner pour choisir "Chargeur" (Feeder)
        scanner.scan(displayImagesOnPage, { 
            "use_asprise_dialog": true, 
    ...
    function displayImagesOnPage(successful, mesg, response) {
        ...
        try {
            ...
            // Logique de r̩cup̩ration de l'image
            ...
            if(format === 'jpg' || format === 'jpeg' || format === 'png') {
                ...
                previewDiv.innerHTML = '<div class="alert success"><i class="fa-solid fa-file-pdf"></i> PDF Multi-pages re̤u</div>';
                fileNameDiv.innerHTML = '<i class="fa-solid fa-check" style="color:#10b981"></i> PDF Multi-pages re̤u !';
                fileNameDiv.style.color = '#10b981';
            }

            // Cr̩ation du fichier pour le formulaire
...
<div class="upload-card" style="max-width: 900px; text-align: left; margin: 0 auto;">
    <form method="POST" enctype="multipart/form-data" id="mailForm">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 30px;">
            ...
            <div>
                <div class="form-group">
                    ...
                    <input type="text" name="object" class="form-control" list="objets_precedents" required placeholder="Saisir ou choisir un objet">
                    <datalist id="objets_precedents">
                        <?php foreach($obj_templates as $ot): ?>
                            <option value="<?= htmlspecialchars($ot['label']) ?>">
                        <?php endforeach; ?>
                        <?php foreach($obj_history as $obj): if(empty($obj)) continue; ?>
        ...
        <div class="form-group" style="margin-top: 30px; border-top: 2px dashed #cbd5e1; padding-top: 20px;">
            ...
            <label style="display:flex; align-items:center; gap:8px; margin:8px 0 14px; color:#334155; font-weight:600;">
                <input type="checkbox" id="withoutDocument" name="without_document" value="1" onchange="toggleDocumentRequirement()">
                Ordre sans document (instruction uniquement)
            </label>
            <div style="display:flex; gap:10px; margin-bottom:15px;">
                <button type="button" class="btn-sm scan-btn" style="background:#0f172a; color:white;" onclick="scanToJpg()">
                    <i class="fa-solid fa-image"></i> Scan Simple (1 page)
                </button>
                <button type="button" class="btn-sm scan-btn" style="background:#3b82f6; color:white;" onclick="scanToPdf()">
                    <i class="fa-solid fa-copy"></i> Scan Multiple (PDF)
                </button>
            </div>

            <div id="scanPreview" style="margin-bottom:10px;"></div>

            <div class="drop-zone" id="mailDropZone" onclick="document.getElementById('fileInput').click();" style="padding: 20px;">
